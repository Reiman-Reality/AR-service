...
export async function getModelAndMarkerNames() {
    try {
        const connection = await pool.getConnection();
        const data = await connection.query("select x_pos, y_pos, z_pos, scale, x_rot, y_rot, z_rot, MARKERS.file_path_one, MODELS.file_path, MODELS.texture_name FROM EVENTS LEFT JOIN MARKERS ON MARKERS.marker_id = EVENTS.marker_id LEFT JOIN MODELS ON MODELS.model_id = EVENTS.model_id;");
        const keys = Object.keys(data);
        const returnData = [];
        for( const key of keys ) {
            if(key === 'meta') {
...
export async function addEvent(event: eventData) {
    try{
        const connection = await pool.getConnection();
        const id = v4();
        const success = await connection.query(`INSERT INTO EVENTS (marker_id, model_id, x_pos, y_pos,z_pos, scale, tag)
        VALUES ( "${event.marker_id}", "${event.model_id}", "${event.x_pos}", "${event.y_pos}", "${event.z_pos}", "${event.scale}", "${event.tag}");`);
        connection.end();
        return id;
    } catch( exception:unknown ){
        console.log(exception);
...
export async function editEvent(event: any) {
    try{
        const connection = await pool.getConnection();
        const id = v4();
        const success = await connection.query(`UPDATE EVENTS 
        SET
